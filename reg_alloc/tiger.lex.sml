...
fun eof() = let val pos = hd(!linePos)
            in
              if(!comment_nest <> 0)
                then ErrorMsg.error pos ("Open comment")
              else if (!string_open)
                then ErrorMsg.error pos ("Open string")
              else ();
              Tokens.EOF(pos,pos)
            end
...
fun yyAction44 (strm, lastMatch : yymatch) = (yystrm := strm;
      (string_open := true; str := ""; YYBEGIN STRING; continue()))
fun yyAction45 (strm, lastMatch : yymatch) = (yystrm := strm;
      (string_open := false; YYBEGIN INITIAL; Tokens.STRING((!str), yypos - size(!str), yypos)))
fun yyAction46 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN ESCAPE; continue()))
fun yyAction47 (strm, lastMatch : yymatch) = (yystrm := strm;
      (lineNum := !lineNum+1; linePos := yypos :: !linePos; ErrorMsg.error yypos ("Newline in string literal is illegal"); continue()))
...
fun yyAction50 (strm, lastMatch : yymatch) = (yystrm := strm;
      (str := !str ^ "\t"; YYBEGIN STRING; continue()))
fun yyAction51 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (str:= !str ^ String.substring(yytext,1,1); YYBEGIN STRING; continue())
      end
fun yyAction52 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (str := !str ^ Char.toString(Char.chr(valOf(Int.fromString yytext))); YYBEGIN STRING; continue())
      end
...
fun yyAction56 (strm, lastMatch : yymatch) = (yystrm := strm;
      (lineNum := !lineNum+1; linePos := yypos :: !linePos; YYBEGIN FORMAT; continue()))
...
fun yyAction63 (strm, lastMatch : yymatch) = (yystrm := strm;
      (comment_nest := !comment_nest - 1;
          if !comment_nest = 0 then YYBEGIN INITIAL else ();
          continue()))
...
fun yyQ47 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction37(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"="
              then yyQ115(strm', yyMATCH(strm, yyAction37, yyNO_MATCH))
       ...
       of NONE => yyAction35(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #">"
              ...
              then if inp = #"="
                  then yyQ116(strm', yyMATCH(strm, yyAction35, yyNO_MATCH))
...
fun yyQ19 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction57(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"0"
              then yyQ25(strm', yyMATCH(strm, yyAction57, yyNO_MATCH))
            else if inp < #"0"
              then yyAction57(strm, yyNO_MATCH)
            else if inp <= #"9"
              then yyQ25(strm', yyMATCH(strm, yyAction57, yyNO_MATCH))
